...
class UpdateNameEdgeStep {
    ...
    @먼저("수정 실패 api 호출을 위한 {string}, {string} 있다")
    fun 수정_실패_api_호출을_위한_있다(id: String, name: String) {
        jsonReq = JsonObject()
    ...
    @그러면("수정 실패 api 호출 결과 {int} 확인한다")
    fun 수정_실패_api_호출_결과_확인한다(status: Int) {
